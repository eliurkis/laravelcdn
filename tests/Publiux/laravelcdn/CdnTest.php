...
namespace Eliurkis\laravelcdn\Tests;
...
class CdnTest extends TestCase
{
    ...
    {
        $this->m_asset = M::mock('Eliurkis\laravelcdn\Contracts\AssetInterface');
        ...
        $this->m_finder = M::mock('Eliurkis\laravelcdn\Contracts\FinderInterface');
        $this->m_finder->shouldReceive('read')
            ->with($this->m_asset)
            ->once()
            ->andReturn(new Collection());

        $this->m_provider = M::mock('Eliurkis\laravelcdn\Providers\Provider');
        $this->m_provider->shouldReceive('upload')
            ->once()
            ->andReturn(true);

        $this->m_provider_factory = M::mock('Eliurkis\laravelcdn\Contracts\ProviderFactoryInterface');
        $this->m_provider_factory->shouldReceive('create')
            ->once()
            ->andReturn($this->m_provider);

        $this->m_helper = M::mock('Eliurkis\laravelcdn\Contracts\CdnHelperInterface');
        $this->m_helper->shouldReceive('getConfigurations')
            ->once()
            ->andReturn([]);

        $this->cdn = new \Eliurkis\laravelcdn\Cdn(
            $this->m_finder,
            $this->m_asset,
    ...
    public function testPushCommand()
    {
        ...
        $finder = new \Eliurkis\laravelcdn\Finder($m_consol);

        $asset = new \Eliurkis\laravelcdn\Asset();

        $provider_factory = new \Eliurkis\laravelcdn\ProviderFactory();
        ...
        $helper = new \Eliurkis\laravelcdn\CdnHelper($m_config);

        $m_console = M::mock('Symfony\Component\Console\Output\ConsoleOutput');
        $m_console->shouldReceive('writeln')
            ->atLeast(2);

        $m_validator = M::mock('Eliurkis\laravelcdn\Validators\Contracts\ProviderValidatorInterface');
        $m_validator->shouldReceive('validate');

        $m_helper = M::mock('Eliurkis\laravelcdn\CdnHelper');

        $m_spl_file = M::mock('Symfony\Component\Finder\SplFileInfo');
        $m_spl_file->shouldReceive('getPathname')
            ->andReturn('Eliurkis\laravelcdn/tests/Eliurkis/laravelcdn/AwsS3ProviderTest.php');
        $m_spl_file->shouldReceive('getRealPath')
            ->andReturn(__DIR__.'/AwsS3ProviderTest.php');

        // partial mock
        $p_aws_s3_provider = M::mock('\Eliurkis\laravelcdn\Providers\AwsS3Provider[connect]',
        [
            $m_console,
            $m_validator,
            $m_helper,
        ...
        $cdn = new \Eliurkis\laravelcdn\Cdn($finder,
            $asset,
            $provider_factory,
            $helper
        );
